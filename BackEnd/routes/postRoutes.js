...
router.get('/',
    optionalVerifyToken,
    (req, res, next) => getPosts(req, res).catch(next))
router.get('/:id',
    optionalVerifyToken,
    validateObjectId('id'),
...
router.delete('/:id',
    ...
    validateObjectId('id'),
    checkPostOwner,
    (req, res, next) => deletePostCtrl(req, res).catch(next))
...
router.post('/:id/images',
    ...
    (req, res, next) => uploadImagesCtrl(req, res).catch(next))
router.delete('/:id/images',
    verifyToken,
    validateObjectId('id'),
    checkPostOwner,
    (req, res, next) => deleteImagesCtrl(req, res).catch(next))

// Comments
router.get('/:id/comments',
    validatePublishedPost('id'),
    (req, res, next) => getCommentsCtrl(req, res).catch(next))
router.post('/:id/comments',
    verifyToken,
    validatePublishedPost('id'),
    validateCommentContent,
    (req, res, next) => createCommentCtrl(req, res).catch(next))
router.put('/:postId/comments/:commentId',
    verifyToken,
    validatePublishedPost('postId'),
    validateObjectId('commentId'),
    validateCommentContent,
    (req, res, next) => updateCommentCtrl(req, res).catch(next))
router.delete('/:postId/comments/:commentId',
    verifyToken,
    validatePublishedPost('postId'),
    validateObjectId('commentId'),
    (req, res, next) => deleteCommentCtrl(req, res).catch(next))
